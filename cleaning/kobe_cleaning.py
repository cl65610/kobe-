...
# The cells in the "shot_made_flag" column that are filled with nulls are the ones
# that we'll be predicitng later. For now, we'll leave them as is while we perform
...
kobe.minutes_remaining.value_counts()

kobe.seconds_remaining.value_counts()
...
season_dummies = pd.get_dummies(kobe.season, drop_first=True)

#join the dummies dataframe onto the original
kobe = kobe.join(season_dummies)
...
kobe.iloc[0].matchup
 ...
 # matches the string and fills with a zero where True, and a one where False
kobe['home'] = np.where(kobe['matchup'].str.startswith('LAL @'), 0, 1)

# Create a column that combines minutes and seconds remaining.
kobe['time_left'] = kobe['minutes_remaining']+(kobe.seconds_remaining/60.0)

# Convert the shot_type column to have integer values for the type of shot.
kobe.shot_type = kobe.shot_type.map({'2PT Field Goal':2, '3PT Field Goal':3})
...
# During the EDA phase of this project, I created the plot below. I've commented it out here to save on runtime
...
action_dummies = pd.get_dummies(kobe.action_type, drop_first=True)
action_cols = action_dummies.columns
kobe = kobe.join(action_dummies)
...
kobe.game_date = kobe.game_date.astype('datetime64')

# Now, we should be able to create this new calculated column using a groupby

games = kobe.groupby('game_id', as_index=False)
...
kobe.game_id.head()
kobe['days_off'] = kobe.game_id
kobe['days_off'] = kobe.game_id.map(days_off_dict)
kobe.days_off.fillna(100, inplace=True)
kobe.days_off = kobe.days_off.astype('timedelta64[D]')
kobe.days_off = kobe.days_off.apply(lambda x: x-1)
kobe.days_off = kobe.days_off.apply(lambda x: x==10 if x>=11 else x)
kobe.days_off = kobe.days_off.apply(lambda x: x+11 if x<0 else x)
kobe.days_off.max()
kobe.days_off.head(15)
